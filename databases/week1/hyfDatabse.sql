...
insert into status (id, name) values (1, 'Not started'), (2, 'In progress'),(3, 'Done') 
...
SELECT * from task where user_id is NULL

select * from task where status_id  = 2


select * from task WHERE status_id = 1 or status_id = 2 order by id DESC

select * from task  where title like "%look%"



select * from task WHERE status_id = 1 and user_id = 1 order by id DESC

select id , description, title  from task  where title like "%_ook%"


SELECT CONCAT("Task: ", title, " - (Status: ", status_id, ")") AS task_info FROM task;


select CONCAT(" Task: ", description , "-(Status :", status_id, ")") AS taskinfo from task where description is NOT NULL
